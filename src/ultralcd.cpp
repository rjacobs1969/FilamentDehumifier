...
void lcd_status_screen() {
    ...
    if (use_click()) {
      ...
      /* ((feedrate_percentage < 100 && new_frm > 100) || (feedrate_percentage > 100 && new_frm < 100)) {
    ...
    feedrate_percentage = constrain(feedrate_percentage, 0, 999);
...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  void lcd_main_menu() {
    START_MENU();
    MENU_BACK(MSG_WATCH);
    MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, MSG_BED, &thermalManager.target_temperature_bed, DRYING_MINTEMP, DRYING_MAXTEMP, watch_temp_callback_bed);
