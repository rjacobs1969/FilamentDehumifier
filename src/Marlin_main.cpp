...
inline void gcode_M121() { endstops.enable_globally(false); }
   ...
   *   M150 I10        ; Increase brightness by 10
   *   M150 D10        ; Decrese brightness by 10
   */
  inline void gcode_M150() {
    ...
    if (parser.seen('I')) {
      pixels.setBrightness(pixels.getBrightness() + parser.has_value() ? parser.value_byte() : 1);
      pixels.show();
    }
...
inline void gcode_M205() {
  ...
  inline void gcode_M206() {
    ...
    #if ENABLED(M206_RELATIVE_Z_OFFSET)
      if (parser.seen('R'))
        set_home_offset(Z_AXIS, home_offset[Z_AXIS] + parser.value_linear_units());
